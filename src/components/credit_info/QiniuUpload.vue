<template>
    <div>
        ...
        <div style="margin: 20px 20px 0 20px;">
            ...
            <a href="javascript:;" class="videoUpload" v-show="videoBtnShow">
                <input type="file" class="videoBtn" id="video" @change="qiniuUpload"  accept="video/*" capture="user">
                开始录制视频
            </a>
        </div>
    ...
    export default {
        ...
        methods:{
            ...
            qiniuUpload(event){
                ...
                this.$axios({
                    method: 'post',
                    url:httpUrl.getVideoToken,               //获取七牛云token
                    data:postData
                }).then((res)=>{
                    console.log(res.data)
                    if(res.data.errCode==200){
                        this.qiniu_token = res.data.retData.uptoken;
                        const key_name = res.data.retData.name;

                        const formdata = new FormData()
                        formdata.append('file', file)
                        formdata.append('key', key_name)
            ...
            lipCodeCheck(){
                ...
                this.$axios({
                    ...
                    if(res.data.errCode==200){
                        ...
                        if(this.transfinite == 1){     //人脸识别校验失败大于6次
                            this.$vux.toast.show({
